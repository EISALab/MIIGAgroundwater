...
C *** STATEMENT MUST BE COMMENTED OUT
      ALLOCATABLE :: GX(:), IG(:), X(:), IX(:), RX(:), IR(:), GZ(:)
      ALLOCATABLE :: Z(:), XHS(:), NIPRNAM(:), EQNAM(:), NAMES(:)
      ALLOCATABLE :: OBSNAM(:)
!     REAL, ALLOCATABLE :: GX(:), X(:), RX(:), XHS(:)
!     DOUBLE PRECISION, ALLOCATABLE :: GZ(:), Z(:)
!     INTEGER, ALLOCATABLE :: IG(:), IX(:), IR(:)
!     CHARACTER, ALLOCATABLE :: EQNAM(:), NIPRNAM(:), NAMES(:), OBSNAM(:)
...
C     Added by Abhishek Singh and Eva Sinha to change program into subroutine
	...
	INTEGER, INTENT(INOUT) :: NCOL2, NROW2, NPER2, UMAERROR
...
c     APPEND_PID called by Eva Sinha
        IF (MYID.EQ.MPROC) THEN
           ...
           OPEN (UNIT=IBUNIT, FILE=FILENAME, STATUS='OLD')
C          OPEN (UNIT=IBUNIT,FILE='MODFLOW.BF',STATUS='OLD')
           FILENAME = 'modbatch.rpt'
           CALL APPEND_PID(FILENAME)
           OPEN (UNIT=IBOUTS,FILE=FILENAME)
...
C      MUST BE COMMENTED OUT
      ...
      MPRD = MPRAR
      IPRD = IPRAR
      ALLOCATE (X(LENX),Z(LENZ),IX(LENIX),XHS(LENXHS), STAT = ERR)
      ALLOCATE (NIPRNAM(IPRAR),EQNAM(MPRAR),NAMES(ND+IPRAR+MPRAR),
     &          OBSNAM(NDAR), STAT = ERR)
C
...
C      COMMENTED OUT
       IF (ITERPK.EQ.1) THEN
         ...
         ALLOCATE (RX(LENRX),IR(LENIR),stat=err)
       ENDIF
C
...
C      IF (IBATCH.GT.0) THEN   ! Commented by Eva Sinha
C       TO USE STATIC MEMORY ALLOCATION, COMMENT OUT THE FOLLOWING
C       DEALLOCATE STATEMENT
        DEALLOCATE (GX,GZ,IG,X,Z,IX,XHS,RX,IR,NIPRNAM,EQNAM)
        DEALLOCATE (NAMES, OBSNAM)
C       GOTO 10                ! Commented by Eva Sinha
c      ENDIF                   ! Commented by Eva Sinha        
C
