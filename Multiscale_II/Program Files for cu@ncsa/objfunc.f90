...
!  use mpi_module
  ...
  integer :: ir,ic,iy,iy_start,iy_end,K,I,J,l,Year, temp_int, iter, i2,err
...
double precision :: t1,t2
  ...
  real :: CONCfrmMT3D(OriNrow1,OriNcol1,Nstress * PeriodLength,NoSpecies)
  REAL :: HEADfrmMODF(OriNrow1,OriNcol1,Nstress)
...
! some changed will have to be made to the input file as well.
  ...
  !allocate(CONCfrmMT3D(OriNrow1,OriNcol1,Nstress * PeriodLength,NoSpecies),stat = err)
  !allocate(HEADfrmMODF(OriNrow1,OriNcol1,Nstress),stat =err)
  ...
  !write(*,*) 'modflow run done'
  CONCfrmMT3D = 0.00d0

  !write(*,*) 'calling mt3dms'
...
!          Print *, "Clean up time", Ny
          ...
          if((FormulNum .ne. 2 .and. (max(QMax1/Alpha,cap_gac(1))/cap_gac(1)-1.) .le.0.001 ) .or. &
               &   (FormulNum .eq. 2 .and. (max(QMax1/Alpha,cap_gac(3))/cap_gac(3)-1.0) .le.0.001 )) then
        ...
        do I = 1,itmp			!when itmp > 0
           ...
           if(q .le. 0) then
              if(q .eq. 0) then
                 ...
                 PmpRate(K,NumExt) = -real(int(10.0*q * togpm+.5)/10.0)
	!			 print *,PmpRate(K,NumExt)
                 if(idbg .eq. 'Y' .or. idbg .eq. 'y') then
              ...
              NumCells(K) = NumCells(K) + 1
              RchRate(K,NumInj) = real(int(10.0*q * togpm+.5)/10.0)
		!	  print *,RchRate(K,NumInj)
...
subroutine opnucn(in,iout,Filename,Numy,OriNrow1,OriNcol1,Cl,C,Ny)
  ...
  do k = 1,Numy
     ...
     !Added by F. Espinoza (4/30/03)
     ! check if any cell is dry and if so then assign a high conc value to that cell
     !    do i = 1,OriNrow1
     !	    do j = 1,OriNcol1
     !               if (c(i,j,iy) .lt. 0 .and. c(i,j,iy) .ne. -999)then
     !                  c(i,j,iy) = 2*max(cmax,cl)
     ...
     !         cmax = maxval(c(:,:,iy)) ! this makes sure that if due to dry cell the conc is changed then rechange the max conc        
...
!	 print *,Qtotal(K),Qtotal(K)/Alpha,InitialGac,ExtraQ
     
     !        See if More Are Needed, Set NewGac(K)
     RequiredGac = Ceiling (ExtraQ / GacCapacity)
	 if((max(Qtotal(K)/Alpha,InitialGac)/InitialGac-1.0).le.0.001) RequiredGac=NumGac
...
!       Step Through Stress Periods K
        do K = 1,Nstress
           ...
           do J = 1,Nwel(K)
              ...
              !           Increase counting
              if((WelZon(K,J) .eq. 1 .and. (max(Q/Alpha,400.)/400.-1.) .gt. 0.001) .or. &
                   &         (WelZon(K,J) .ne. 1 .and. (max(Q/Alpha,1000.)/1000.-1.0 ).gt. 0.001)) then
...
subroutine VCEfunc (Ny, Nwel, PumpRate, MxWel1, Rate, Nstress, &
  ...
  do K = 1, Nstress_Cleanup
     ...
     do I = 1, Nwel(K)
        ...
        if ( (max(PumpRate(K,I)/Alpha,400.)/400.-1.0) .le. 0.001) then
...
!  do i = 1, OriNRow1
!     do j = 1, OriNCol1
!        if (C(i,j,Numy) .gt. cmax) cmax = C(i,j,Numy)
!     end do
!  end do
