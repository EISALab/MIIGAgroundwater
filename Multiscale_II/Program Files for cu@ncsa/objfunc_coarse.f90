subroutine Obj_Func_Coarse(Fitness, Total_cost, Pen_Rdx, Pen_Tnt, gridflagx,gridflagy,drycell_c, drycell_h)
  ...
  real :: CONCfrmMT3D(Nrow1,Ncol1,Nstress * PeriodLength,NoSpecies)
  real :: HEADfrmMODF(Nrow1,Ncol1,Nstress)
  ...
  !allocate(CONCfrmMT3D(Nrow1,Ncol1,Nstress * PeriodLength,NoSpecies),stat=err)
  !allocate(HEADfrmMODF(Nrow1,Ncol1,Nstress),stat=err)
...
4         format(/5x,'The initial plume area, both RDX and TNT (ft*ft): ', &
	  ...
	  do k = 1,Numy
             ...
             !    do i=1,Nrow1
             !       do j=1,Ncol1
             !          if (c(i,j,iy) .lt. 0 .and. c(i,j,iy) .ne. -999 .and. abs(c(i,j,iy)) .gt. 0.01)then
