program uma
  ...
  save
     ...
     integer :: execval, gridflagx(num_groups), gridflagy(num_groups), runend(num_groups)
	...
	do group_id = 1, 3
		if (group_id .eq. 1 ) then
			gridflagx(group_id) = 2 ! 2 refers to fine grid which has 132rows and 125 cols
			gridflagy(group_id) = 2 ! 1 refers to coarse grid which has 66 rows and  64 cols
		else
			gridflagx(group_id) = 1 ! group_1 is for FINE GRID and group_2 and group_3 are for COARSE GRID
			gridflagy(group_id) = 1
		end if
	end do
	...
	do group_id = 1, 3
		call remedwell(gridflagx, gridflagy, group_id)    
	end do
...
!Main processing loop for the simple genetic algorithm
	...
	do group_id = 1, num_groups
		write(*,*) 'npopsiz, maxgen, gridflagx(group_id), feed, irestrt, best_interval'
		write(*,*)  npopsiz, maxgen, gridflagx(group_id), feed, irestrt, best_interval
		write(file_num(group_id,1),*) 'npopsiz, maxgen, gridflagx(group_id), feed, irestrt, best_interval'
		write(file_num(group_id,1),*)  npopsiz, maxgen, gridflagx(group_id), feed, irestrt, best_interval
	...
	do igen = istart,maxgen+istart-1  ! GENERATIONS LOOP
		...
		if (runend(1) .eq. 0)	call DO_SGA(igen,idum1,ran1,1, gridflagx, gridflagy)
		if (runend(2) .eq. 0)	call DO_SGA(igen,idum2,ran2,2, gridflagx, gridflagy)
		if (runend(3) .eq. 0)	call DO_SGA(igen,idum3,ran3,3, gridflagx, gridflagy)         
		...
		if ((comm_interval .eq. 0) .and. (commend(1) .eq. 0) )	call update_fine_population(1,2,3)
		if ((comm_interval .eq. 0) .and. (commend(2) .eq. 0) )	call update_coarse_population(2,3)
		if ((comm_interval .eq. 0) .and. (commend(3) .eq. 0) )  call update_coarse_population(3,2)
...
!###################################################################
subroutine DO_SGA(igen, idum1, ran1, group_id, gridflagx, gridflagy)
!Main processing loop for the simple genetic algorithm
	...
	external ran1
	integer, intent(in):: group_id, igen, idum1, gridflagx(num_groups), gridflagy(num_groups)
	integer :: n,j,p, nnchild
	...
	double precision ::t1, t2, t3, totaltime, converge(num_groups)
	...
	if(igen.eq.1) then           !---------IGEN=1 LOOP BEGINS--------------
		...
		call sGA_evalout(npopsiz, gridflagx, gridflagy, group_id)

		call cpu_time(t2)
		write(*,1115) 0

		do j = 1,npopsiz
			write(file_num(group_id,2), 1113) j, fitness(group_id,1,j),cost(group_id,j),pen1(group_id,j),pen2(group_id,j),pen3(group_id,j),pen4(group_id,j),(nint(parent(group_id,j,k)), k = 1, 7),&
			& (parent(group_id,j,k), k = 8, 11),(nint(parent(group_id,j,k)), k = 12, 18),&
			& (parent(group_id,j,k), k = 19, 22),(nint(parent(group_id,j,k)), k = 23, 29),drycell(group_id,1,j),drycell(group_id,2,j)
			write(*, 1113) j, fitness(group_id,1,j),cost(group_id,j),pen1(group_id,j),pen2(group_id,j),pen3(group_id,j),pen4(group_id,j),(nint(parent(group_id,j,k)), k = 1, 7),&
			& (parent(group_id,j,k), k = 8, 11),(nint(parent(group_id,j,k)), k = 12, 18),&
			& (parent(group_id,j,k), k = 19, 22),(nint(parent(group_id,j,k)), k = 23, 29),drycell(group_id,1,j),drycell(group_id,2,j)
		end do
		...
		write(*,2223) 0,av0(group_id), std(group_id), std(group_id)/av0(group_id),best(group_id),nbest(group_id)
		write(*,*)
2223    format(i4,2x, f18.8,2x, f25.12,2x, f18.12,2x, f15.2,2x, i6)
	...
	if (igen .le. maxgen+istart-1) then ! *** check for i
		...
		if(itourny.eq.1) then
			...
			do j = 1, npopsiz
				write(file_num(group_id,3),1113) j, fitness(group_id,1,j),cost(group_id,j),pen1(group_id,j),pen2(group_id,j),pen3(group_id,j),pen4(group_id,j),(nint(parent(group_id,j,k)), k = 1, 7),&
				& (parent(group_id,j,k), k = 8, 11),(nint(parent(group_id,j,k)), k = 12, 18),&
	...
	call BestValue(best, group_id)
	call Avg(av0,std,nbest,best, group_id)
	call stats(fitness, mean, variance, converge, group_id)
	write (file_num(group_id,1),2224) igen, av0(group_id), std(group_id), std(group_id)/av0(group_id), best(group_id), nbest(group_id), converge(group_id)
	write(*,*)
	write (*,2224) igen, av0(group_id), std(group_id), std(group_id)/av0(group_id), best(group_id), nbest(group_id), converge(group_id)
	write(*,*)
...
2224    format(i4,2x, f18.8,2x, f25.12,2x, f18.12,2x, f15.2,2x, i6, 2x, f15.2)
...
!#######################################################################
subroutine sGA_evalout(npop, gridflagx, gridflagy, group_id)
use condorio
	...
	integer, intent(in) :: npop, gridflagx(num_groups), gridflagy(num_groups), group_id 
...
1116      format(i4,2x,7i5,2x,7i3,3x,7i3)
	...
	do z = 1,npop
		...
		call BroadcastSlave(z, npop+1, gridflagx(group_id), gridflagy(group_id) )
	enddo
...
subroutine Avg(av0,std,nbest,best, group_id)
	...
	integer, intent(in) :: group_id
	double precision :: av0(num_groups),std(num_groups),best(num_groups),dif,tol
	integer :: nbest(num_groups),mm
...
subroutine stats (fit, avg, var, conv, group_id) 
...
! possible locations'
   ...
   subroutine remedwell (gridflagx, gridflagy,group_id)

     IMPLICIT none
     save

     integer, intent(in) :: gridflagx(num_groups), gridflagy(num_groups), group_id
     ...
     if ( (gridflagx(group_id).eq.1) .and. (gridflagy(group_id).eq.1))then
        open(unit = 36, file='uma_coarse.txt',status='old')
     endif
     if ( (gridflagx(group_id).eq.2) .and. (gridflagy(group_id).eq.2))then
...
10          ninj1=0
     ...
     20    totwell=2*(k-1)*ninj1+ninj2+next
         ...
         nw1=nint(parent(group_id,indiv,1))*parent(group_id,indiv,newwells+1+4)
         nw2=nint(parent(group_id,indiv,2))*parent(group_id,indiv,newwells+2+4)
...
111 format(i3,2x,60i2)
	...
	!do i=1,npopsiz
	!	write(38,100) i,(iparent(group_id,j),j=1,nchrome)
	!enddo
...
subroutine BestValue(best, group_id)
	...
	do j=1,npopsiz
		if(optflag.eq.1) then
			...
			if(fitness(group_id,1,j).gt.best(group_id)) then 
				best(group_id) = fitness(group_id,1,j)
				jbest(group_id,1) = j
				!Writing the coded form of the best individual
				do m = 1,nchrome
...
subroutine bestcalc(group_id)
	...
	integer, intent(in) :: group_id
	integer :: j,k,i,l,m,f,g,p,remaining, z, num
	...
	do p = 1, npopsiz
		num = num + 1
		best(group_id,p)= fitness(group_id,1,p)
		jbest(group_id,p)= p
		do m= 1, nchrome
			ibest(group_id, p,m)= iparent(group_id,p,m)
		end do
	end do
...
subroutine update_fine_population(group_r, group_s1, group_s2) 
...
end subroutine update_fine_population
!########################################################################
! This subroutine updtaes the population with the best chromosomes
! received from other groups

subroutine update_coarse_population(group_r, group_s1) 

	IMPLICIT none
	save

	integer, intent(in) :: group_r, group_s1
	integer :: q,r, j, k,n
...
1234	format(7i4,2x,4(f5.3,2x),7i2,3x,4(f5.3,2x),7i2)
end subroutine update_coarse_population
!########################################################################
