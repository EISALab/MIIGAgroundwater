...
void CServerApp::SetPriority( int nStateFlags )
{
	...
	list<HPROCESS>::iterator iter = m_lstProcHandles.begin();
...
void CServerApp::RegistProc( HPROCESS hProc )
{
	Lock();
...
void CServerApp::UnregistProc( HPROCESS hProc )
{
	Lock();
...
void CServerApp::RunServer()
{
	...
	CreateThread( (THREAD_FUNC)PingServThread, NULL, 0, NULL );
