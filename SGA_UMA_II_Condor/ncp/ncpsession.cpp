...
int CClientSession::Connect( SOCKADDR_IN* srvAddr, const char* strUser, const char* strPass )
{
	...
	int timeout = 20000;	//wait for 20 seconds for the login. if no information, then quit login.
	int status = setsockopt( m_sock, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout, sizeof(timeout) );

	if( nRet!=0 )return E_NOHOST;

	nRet = DoLogOn( strUser, strPass );

	timeout = 0;
	status = setsockopt( m_sock, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout, sizeof(timeout) );
