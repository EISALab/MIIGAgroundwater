...
protected:
	...
	bool OnSetIdle( CNcpMessage* pMsg, int nBufSize );
...
#define SYSTEM_SUSPEND_SERVICE		0x08

inline bool IsSystemFree( int nSysStatFlags )
{
	return IsClr(nSysStatFlags, SYSTEM_SUSPEND_SERVICE) && (
				IsClr(nSysStatFlags, SYSTEM_USER_LOGON) || IsSet(nSysStatFlags, SYSTEM_USER_LOCK) ||
