MODULE PARAMS
	...
	!variables for iiga method
	integer, parameter :: num_groups = 3
	integer, parameter :: best_interval = 10! generation interval for communication between groups (migration frequency)
	integer, parameter :: maxswaps = 10  ! twice the maximum no. of best individuals communicated from any group.
	integer, parameter :: group_c = 5 ! no. of best chromosomes accepted by one group from another group.
	...
	real, parameter :: best_frac = 0.20 ! make this 12.5% for pop size of 120

	! Proportion of fine individuals selected from fine section of population, for final mu+lambda
	! selection scheme.
	double precision :: best_selec_frac(num_groups) = 0.50
